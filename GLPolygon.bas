...
Public Sub Create()
	...
	TJO.InitializeNewInstance("javafx.scene.shape.Polygon",Null)
End Sub

'Creates a new instance of Polygon. Points should be an Array of Doubles
Public Sub Create2(Points() As Double)
	TJO.InitializeNewInstance("javafx.scene.shape.Polygon",Array(Points))
End Sub

'Gets the coordinates of the Polygon vertices.
Public Sub getPoints As Double()
	
	Dim OL As JavaObject = TJO.RunMethod("getPoints",Null)
	Dim Result(OL.RunMethod("size",Null)) As Double
	Dim Iterator As JavaObject = OL.RunMethod("iterator",Null)
	Dim i As Int = 0
	Do While Iterator.RunMethod("hasNext",Null)
		Result(i) = Iterator.RunMethod("next",Null)
		i=i+1
	Loop
	Return Result
End Sub
