...
Public Sub Subtract(Shape1 As GLCubicCurve, Shape2 As GLCubicCurve) As GLShape
	Dim S As GLShape
	S.Initialize
	Return S.Subtract(Shape1,Shape2)
End Sub
'Returns a new Shape which is created as a union of the specified input shapes.
Public Sub Union(Shape1 As GLCubicCurve, Shape2 As GLCubicCurve) As GLShape
	Dim S As GLShape
	S.Initialize
	Return S.Union(Shape1,Shape2)
End Sub
'Returns a new Shape which is created as an intersection of the specified input shapes.
Public Sub Intersect(Shape1 As GLCubicCurve, Shape2 As GLCubicCurve) As GLShape
	Dim S As GLShape
	S.Initialize
...
End Sub
Private Sub MinX As Double
	Return TJO.RunMethodJO("getLayoutBounds",Null).RunMethod("getMinX",Null)
End Sub